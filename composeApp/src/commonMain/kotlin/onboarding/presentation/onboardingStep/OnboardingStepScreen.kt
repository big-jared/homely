...
abstract class OnboardingStep(private val isFinal: Boolean = false, val displayOnly: Boolean = false) : Screen {
    ...
    override fun Content() {
        ...
        Box {
            Column(modifier = Modifier.fillMaxSize().padding(20.dp)) {
                Column(
                    modifier = Modifier.align(if (isHorizontalLayout()) Alignment.Start else Alignment.CenterHorizontally).widthIn(max = 500.dp)
                ...
                Button(modifier = Modifier.align(Alignment.CenterHorizontally).widthIn(max = 420.dp)
    ...
    protected suspend fun continueToNextStep(navigator: Navigator, viewModel: OnboardingViewModel, onFailure: (OnboardingResult.Failure) -> Unit = {}) {
